...
class NotificationPageCubit extends Cubit<NotificationPageState> {
  ...
  NotificationPageCubit() : super(const NotificationPageState(0));
  ...
  void updatePageIndicator(int index) {
    ...
    if (index != state.currentPageIndex) {
      emit(NotificationPageState(index));
    }
  }
  ...
  void next() {
    ...
    if (currentIndex < totalPages - 1) {
      ...
      emit(NotificationPageState(nextPageIndex));
  ...
  void previous() {
    ...
    if (currentIndex > 0) {
      ...
      // Emit the new state to update the indicator
      emit(NotificationPageState(previousPageIndex));
  ...
  void skipPage() {
    ...
    // Emit the new state to update the indicator
    emit(NotificationPageState(lastPageIndex));
